...
| D007 | M010/S04/T01 | pattern | Header mobile layout approach | Stack logo+nav above controls at max-md breakpoint rather than hamburger menu | Keeps all functionality visible on mobile without JS toggle state. The Bloomberg dashboard has ~8 header controls — hiding them behind a hamburger would make the monitoring workflow worse. Two-row stacking keeps everything accessible. | Yes — if header gets significantly more controls | agent |
| D008 | M010/S04 | requirement | R011 | validated | T02 added ARIA attributes to 20 component files, raising the aria-* count from 11 to 51 (threshold ≥50). Added aria-label to icon-only buttons and inputs, aria-pressed to toggle buttons, aria-expanded to collapsibles, role='status' to live regions, role='group' to toggle containers. All changes additive-only, verified by rg count + build + tests. | Yes | agent |
