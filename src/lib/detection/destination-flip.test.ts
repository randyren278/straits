...
describe('detectCompositeDiversions', () => {
  ...
  it('writes composite_diversion when a flip is followed by going_dark', async () => {
    ...
    const { upsertAnomaly } = await import('@/lib/db/anomalies');
    ...
    expect(count).toBe(1);
    expect(upsertAnomaly).toHaveBeenCalledTimes(1);
    const arg = vi.mocked(upsertAnomaly).mock.calls[0][0];
  ...
  it('writes nothing for a lone routine flip with no following evasion', async () => {
    ...
    expect(count).toBe(0);
    expect(upsertAnomaly).not.toHaveBeenCalled();
  });
