...
export default function AnalyticsPage() {
  ...
  // Filter panel: collapsed by default on phones; always shown from tablet up via CSS (roomy:block).
      ...
      <main className="p-6 max-w-7xl mx-auto phone:p-3 phone:pb-[calc(var(--straits-nav-h)+1rem)]">
        ...
        <div className="mb-6">
          <button
            ...
            className="roomy:hidden w-full flex items-center justify-between min-h-[44px] px-3 border border-amber-500/20 bg-gray-900 text-xs font-mono uppercase tracking-widest text-amber-500"
          >
            <span>Filters · {timeRange.toUpperCase()} · {viewMode === 'chokepoint' ? `${selectedChokepoints.length} CP` : 'ROUTE'} · {priceSymbol}</span>
            <span>{filtersOpen ? '▾' : '▸'}</span>
          </button>
          <div className={`${filtersOpen ? 'flex' : 'hidden'} roomy:flex flex-wrap gap-4 items-center phone:mt-2 p-3 bg-gray-900 border border-amber-500/20`}>
          <div>
          ...
          <div>
            ...
            <div className="flex gap-1 phone:gap-2">
              {(['chokepoint', 'route'] as const).map((mode) => (
                <button
                  ...
                  className={
                    viewMode === mode
                      ? 'border border-amber-500 bg-amber-500/10 text-amber-500 text-xs font-mono uppercase tracking-wider px-2 py-1 phone:min-h-[44px] phone:px-3 tablet:min-h-[44px] inline-flex items-center'
                      : 'border border-gray-700 text-gray-400 hover:border-amber-500/50 hover:text-gray-300 text-xs font-mono uppercase tracking-wider px-2 py-1 phone:min-h-[44px] phone:px-3 tablet:min-h-[44px] inline-flex items-center'
                  }
                >
                  {mode}
                </button>
